...
from two_pointers.pair_with_target_sum import pair_with_target_sum_hashtable
...
class TestPairWithTargetSum(TestCase):
    ...
    def test_pair_with_target_sum_empty_array(self):
        with self.assertRaises(ValueError):
            pair_with_target_sum_hashtable([], 7)

    def test_pair_with_target_sum_equal_first_last(self):
        self.assertEqual(pair_with_target_sum_hashtable(
            [1, 2, 3, 4, 6], 7), [0, 4])
        self.assertEqual(pair_with_target_sum_hashtable(
            [3, 5, 12, 17], 20), [0, 3])
